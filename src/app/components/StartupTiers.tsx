            ...
            >
              {tier.featured && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
                  <Badge className="bg-[#06b6d4] text-white px-4 py-1">Most Popular</Badge>
                </div>
              )}
              
              <Card className={`p-8 h-full border-2 ${tier.featured ? 'border-[#06b6d4] shadow-xl scale-105' : 'hover:border-[#06b6d4]/30'} transition-all duration-300 bg-white`}>
                ...
                <div className="text-center mb-6">
                  <Badge variant="outline" className="mb-2 text-[#0A1F44] border-[#0A1F44]/20">
                    {tier.tier}
                  </Badge>
                  ...
                  {tier.features.map((feature, featureIndex) => (
                    <div key={featureIndex} className="flex items-start gap-3">
                      <div className="w-5 h-5 rounded-full bg-[#06b6d4]/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <svg className="w-3 h-3 text-[#06b6d4]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
